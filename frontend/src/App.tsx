      ...
      {(!networkOk || !account || (account && leaderboard.find(e => e.address.includes('You'))?.total === '0.0')) && (
        <div className="bg-slate-900 text-white min-h-[140px] px-8 flex flex-col md:flex-row items-center justify-between gap-6 sticky top-0 z-60 shadow-2xl border-b border-white/10 py-6">
          <div className="flex items-center gap-5 max-w-xl">
            ...
            <div>
              <p className="font-black text-lg tracking-tight text-white mb-1">DASHBOARD SETUP REQUIRED</p>
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest leading-relaxed">
                Connect to the **Account with 10,000 BNB** to start testing.
              </p>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
             <div className="bg-white/5 border border-white/10 p-3 rounded-2xl flex items-center gap-3 w-full sm:w-auto">
               <code className="text-[10px] font-mono text-emerald-400 font-bold">Key: 0x5de411...365a</code>
               <button 
                onClick={() => {
                  navigator.clipboard.writeText("0x5de4111afa1a4b94908f83103eb1f1706367c2e68ca870fc3fb9a804cdab365a");
                  alert("✅ KEY COPIED!\n\n1. Open MetaMask\n2. Circle Icon -> Import Account\n3. Paste this key\n4. Make sure to SELECT this new account!");
                }}
                className="bg-emerald-500 hover:bg-emerald-400 text-white px-4 py-2 rounded-xl text-[10px] font-black transition-all shadow-lg shadow-emerald-500/20"
               >
                 COPY KEY
               </button>
             </div>
             <button 
              onClick={fixMetaMask}
              className="bg-white text-slate-900 hover:bg-slate-100 px-8 py-4 rounded-2xl text-xs font-black transition-all active:scale-95 w-full sm:w-auto shadow-xl"
             >
               FIX METAMASK (AUTO-SETUP)
             </button>
          </div>
        </div>
      )}
